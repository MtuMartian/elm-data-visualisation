...
ticks model iter total =
  if iter >= total then
    ...
    let
      height = toString (model.height * iter // total)
    in
...
bars model iter =
  if iter >= (List.length model.data) then
    ...
    let
      bar = Array.get iter (Array.fromList (model.data))
    in
      case bar of
        Just bar ->
          case model.range of
              ...
              let
                highlightModifier =
                  if bar.isHighlighted then
                    ...
                    0
                min = Tuple.first range
                max = Tuple.second range
                normalizer = (bar.value - min) / (max - min)
                height = model.height - (truncate( (toFloat model.height) * normalizer ) )
